...
public class NameTagsModule extends Module {
	...
	public float[] getYawAndPitch(Entity entity) {
		...
		double d3 = (entity.getEntityBoundingBox().minY + entity.getEntityBoundingBox().maxY) / 2.0 - mc.player.posY;
	...
	public boolean isFacingAtEntity(Entity entity, double d) {
		EntityPlayerSP entityPlayerSP = mc.player;
		float[] arrf = getYawAndPitch(entity);
		float f = arrf[0];
		float f2 = arrf[1];
		return AngleDistance(entityPlayerSP.rotationYaw, f) < (d /= 4.5)
				&& AngleDistance(entityPlayerSP.rotationPitch, f2) < d;
	}
	...
	public void onRender3D(Render3DEvent ytROIMjqOZSKSkb2) {
		if (getState()) {
			...
			list.stream().filter((entityPlayer) -> !(entityPlayer == null || entityPlayer == mc.player))
					.forEachOrdered((entityPlayer) -> {
						try {
							...
							double d4 = entityPlayer.lastTickPosX + (entityPlayer.posX - entityPlayer.lastTickPosX) * f
									- d;
							double d5 = entityPlayer.lastTickPosY + (entityPlayer.posY - entityPlayer.lastTickPosY) * f
									- d2;
							double d6 = entityPlayer.lastTickPosZ + (entityPlayer.posZ - entityPlayer.lastTickPosZ) * f
									- d3;
							renderNametag(entityPlayer, d4, d5, d6);
	...
	public void renderArmor(EntityPlayer entityPlayer) {
		...
		n = nonNullList2.stream().filter((object) -> !(object == null)).map((_item) -> 8).reduce(n,
		...
		if (entityPlayer.getHeldItem(EnumHand.MAIN_HAND) != null) {
			...
			if (itemStack.hasEffect()
					&& (itemStack.getItem() instanceof ItemTool || itemStack.getItem() instanceof ItemArmor)) {
				Field object;
	...
	public void renderNametag(EntityPlayer entityPlayer, double d, double d2, double d3) {
		double d4 = getSize(entityPlayer) * -0.0225;
		...
		int n = bl
				? fontRenderer.getStringWidth(String
						.valueOf(new StringBuilder().append(String.valueOf(String.valueOf(getPlayerName(entityPlayer))))
								.append(" ").append(getHealth(entityPlayer))))
						/ 2
				: fontRenderer.getStringWidth(getPlayerName(entityPlayer)) / 2;
		GLHelper.drawBorderedRect(-n - 2, -mc.fontRenderer.FONT_HEIGHT - 6, n + 2,
				(float) (mc.fontRenderer.FONT_HEIGHT + 0.5), 1.0f, -1879048192, 1879048192);
		GL11.glDisable(2929);
		if (!bl)
			fontRenderer.drawStringWithShadow(getPlayerName(entityPlayer), n, 0.0f, 15790320);
		else if (bl) {
			fontRenderer
					.drawStringWithShadow(getPlayerName(entityPlayer),
							-fontRenderer.getStringWidth(String.valueOf(new StringBuilder()
									.append(String.valueOf(String.valueOf(getPlayerName(entityPlayer)))).append(" ")
									.append(getHealth(entityPlayer)))) / 2,
							0.0f, 15790320);
			fontRenderer.drawStringWithShadow(getHealth(entityPlayer), (fontRenderer
					.getStringWidth(String.valueOf(
							new StringBuilder().append(String.valueOf(String.valueOf(getPlayerName(entityPlayer))))
									.append(" ").append(getHealth(entityPlayer))))
					- fontRenderer.getStringWidth(getHealth(entityPlayer)) * 2) / 2, 0.0f,
					getHealthColorHEX(entityPlayer));
		}
		GL11.glEnable(2929);
