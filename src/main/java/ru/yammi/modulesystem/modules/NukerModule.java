...
public class NukerModule extends Module {
	...
	public NukerModule() {
		...
		getValues().add(new SliderValue("Radius", 0.0f, 5.0f, 0.1f));
						...
						for (float f = radius; f >= -radius; f -= 1.0f)
							for (float f2 = radius; f2 >= -radius; f2 -= 1.0f)
								for (float f3 = -radius; f3 <= radius; f3 += 1.0f) {
									...
									IBlockState iBlockState = Minecraft.getMinecraft().world
											.getBlockState(new BlockPos(n, n2, n3));
									...
									Minecraft.getMinecraft().getConnection()
											.sendPacket(new CPacketPlayerDigging(
													...
													new BlockPos(n, n2, n3), EnumFacing.DOWN));
									Minecraft.getMinecraft().getConnection()
											.sendPacket(new CPacketPlayerDigging(
