...
public class ParticleSystem {
	...
	public void render() {
		...
		for (Particle jEMXLMISzubihMw2 : particleList) {
			...
			GL11.glEnd();
			int n = Mouse.getEventX() * Minecraft.getMinecraft().currentScreen.width
					/ Minecraft.getMinecraft().displayWidth;
			int n2 = Minecraft.getMinecraft().currentScreen.height - Mouse.getEventY()
					* Minecraft.getMinecraft().currentScreen.height / Minecraft.getMinecraft().displayHeight - 1;
			float f = 0.0f;
			Particle jEMXLMISzubihMw3 = null;
			for (Particle jEMXLMISzubihMw4 : particleList) {
				float f2 = jEMXLMISzubihMw2.getDistanceTo(jEMXLMISzubihMw4);
				if (f2 > dist
						|| distance(n, n2, jEMXLMISzubihMw2.getX(), jEMXLMISzubihMw2.getY()) > dist
								&& distance(n, n2, jEMXLMISzubihMw4.getX(), jEMXLMISzubihMw4.getY()) > dist
						|| f > 0.0f && f2 > f)
					continue;
				f = f2;
			...
			if (jEMXLMISzubihMw3 == null)
				continue;
			float f3 = Math.min(1.0f, Math.min(1.0f, 1.0f - f / dist));
			drawLine(jEMXLMISzubihMw2.getX(), jEMXLMISzubihMw2.getY(), jEMXLMISzubihMw3.getX(), jEMXLMISzubihMw3.getY(),
					1.0f, 1.0f, 1.0f, f3);
		}
		GL11.glPushMatrix();
	...
	public void tick(int n) {
		if (Mouse.isButtonDown(0))
			addParticles(1);
		particleList.forEach((jEMXLMISzubihMw2) -> {
