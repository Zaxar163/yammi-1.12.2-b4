...
public class ConfigHelper {
	...
	static {
		path = String.valueOf(new StringBuilder().append(System.getProperty("user.home")).append(File.separator)
				.append("yammi-1.12.2-b4.json"));
	}
			...
			try (FileReader fileReader = new FileReader(file)) {
				...
				if (xrayBlocksArray != null)
					for (int i = 0; i < xrayBlocksArray.size(); ++i)
						XRayBlockModelRenderer.xrayBlocks
								.add(xrayBlocksArray.get(i).getAsJsonObject().get("id").getAsInt());
	...
	private static void setSliderX(Module module, String valueTag, int n) {
		if (module.getValues().size() > 0)
			module.getValues().stream().filter((sliderValue) -> sliderValue.getName().equals(valueTag))
					.forEachOrdered((sliderValue) -> {
